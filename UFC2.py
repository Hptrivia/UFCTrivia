...
friend_templates_by_theme = {
    "WWE": {
        "Stone Cold":    "Austin 3:16 says I just whooped your ass. The answer is {answer}.",
        "The Rock":      "If you smell what The Rock is cooking, The answer is {answer}.",
        "Triple H":      "Time to play the game: the answer is {answer}.",
        "The Undertaker":"Rest in peace: the answer is {answer}.",
        "Shawn Michaels":"Heartbreak Kid says the answer is {answer}.",
    },
    "MMA": {
        "Alex Pereira":    "{answer}. Chama",
        "Conor McGregor":  "I’m not here to take part, I’m here to take over. The answer is {answer}.",
        "Israel Adesanya": "The Last Stylebender’s final word is {answer}.",
        "GSP":             "I am the athlete as well as the martial artist. The answer is {answer}.",
        "Jon Jones":       "Bones says there is no stopping him—just like {answer}.",
    },
    "SOCCER(EPL)": {
        "Erling Haaland":  "Like my goals, the answer is unstoppably {answer}.",
        "Mohamed Salah":   "You’ll never walk alone with this answer: {answer}.",
        "Kevin De Bruyne": "Vision like mine always sees the answer: {answer}.",
        "Bukayo Saka":     "My creativity leads to {answer}.",
        "Harry Kane":      "Harry scores again: the answer is {answer}.",
    }
}
...
with gr.Blocks() as demo:
    ...
    with gr.Column(visible=True) as theme_page:
        gr.Markdown("## 🎯 Sports Trivia")
        gr.Markdown("## Select the sport you want to play")
        friends_btn   = gr.Button("WWE")
        naruto_btn    = gr.Button("MMA")
        avengers_btn  = gr.Button("SOCCER(EPL)")
    ...
    with gr.Column(visible=False) as theme_menu:
        ...
        rules_md      = gr.Markdown(
            ...
            "1.**Click on the share icon on the top right of your browser, scroll to the button and click add to home screen. This will enable you have the game as an app icon on your phone**\n\n",
            visible=True
        )
    ...
    friends_btn.click(
        fn=lambda: ("WWE", gr.update(visible=False), gr.update(visible=True), gr.update(value="## WWE Trivia"), gr.update(visible=True), gr.update(visible=True)),  
        outputs=[selected_theme, theme_page, theme_menu, theme_heading, back_to_themes, shop_btn]
    )
    naruto_btn.click(
        fn=lambda: ("MMA", gr.update(visible=False), gr.update(visible=True), gr.update(value="## MMA Trivia"), gr.update(visible=True), gr.update(visible=True)),
        outputs=[selected_theme, theme_page, theme_menu, theme_heading, back_to_themes, shop_btn]
    )
    avengers_btn.click(
        fn=lambda: ("SOCCER(EPL)", gr.update(visible=False), gr.update(visible=True), gr.update(value="## EPL Trivia"), gr.update(visible=True), gr.update(visible=True)),
        outputs=[selected_theme, theme_page, theme_menu, theme_heading, back_to_themes, shop_btn]
    )
